...
(function(c){
    ...
    c.extend(c.jqx._jqxListMenu.prototype,{
        ...
        _renderBackButton:function(){
            if(!this._backButton){
                this._backButton=c('<input type="button" value="'+this.backLabel+'"/>');
        ...
        _renderLi:function(d){
            ...
            if(this.showNavigationArrows||this.alwaysShowNavigationArrows){
                ...
                if(this.alwaysShowNavigationArrows||(((g.length>0)&&(/(listmenu)/).test(g.data("role")))||((e.length>0)&&(/(listmenu)/).test(e.data("role"))))){
                    ...
                    f.addClass(this.toThemeProperty("icon-arrow-right"));
                    f.appendTo(d)
                }
            }
        },
        _removeLiEventHandlers:function(d){
            var f=this.isTouchDevice();
            var e=(!f?"mousedown":"touchstart")+".listmenu";
            var g=(!f?"mouseup":"touchend")+".listmenu";
            this.removeHandler(c(d),e);
            this.removeHandler(c(document),g+"."+d.id)
        },
        ...
        _addLiEventHandlers:function(d){
            d=c(d);
            var f=this,k=this.toThemeProperty("jqx-listmenu-arrow-right-pressed"),j=d.children(".jqx-listmenu-arrow-right");
            var g=f.isTouchDevice();
            var e=(!g?"mousedown":"touchstart")+".listmenu";
            var i=(!g?"mouseup":"touchend")+".listmenu";
            var h=null;
            if(!(/(separator|readonly)/).test(d.data("role"))&&!this.readOnly){
                this.addHandler(d,"dragstart",function(){
                    return false
                });
                this.addHandler(d,e,function(l){
                    if(!f.disabled){
                        h=l.target;
                        if(d.find('div[data-role="content"]').length==0){
                            d.addClass(f.toThemeProperty("jqx-fill-state-pressed"));
                            j.addClass(k)
                        }
                    }
                });
                this.addHandler(d,i,function(l){
                    if(!f.disabled){
                        if(h==l.target||!g){
                            f.next(d)
                        }
                ...
                this.addHandler(c(document),i+"."+d[0].id,function(){
                    if(!f.disabled){
                        d.removeClass(f.toThemeProperty("jqx-fill-state-pressed"));
                        j.removeClass(k)
                    }
                })
            }
        ...
        _slideChange:function(g,e,f,h){
            ...
            this._initSlide(g,e);
            g.animate({
                "margin-left":-g.width()-parseInt(g.css("margin-right"),10)||0
            },f,"easeInOutSine");
            e.animate({
                "margin-left":0
            },f,"easeInOutSine",function(){
                d._slideEnd(g,e);
                h.call(d,c(this))
            })
        },
        _initSlide:function(e,d){
            e.width(e.width());
            d.css({
                marginTop:-(e.outerHeight(true)),
                marginLeft:e.width()+(parseInt(e.css("margin-right"),10)||0),
                display:"block",
                height:"auto",
                width:e.width()
            })
        ...
        _filter:function(g){
            ...
            for(var e=0;e<f.length;e+=1){
                if(!this.filterCallback(f[e].innerText,g)){
