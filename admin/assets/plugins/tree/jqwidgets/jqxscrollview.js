...
(function(b){
    ...
    b.extend(b.jqx._jqxScrollView.prototype,{
        ...
        _addEventListeners:function(){
            var c=this;
            this.addHandler(this._innerWrapper,this._getEvent("mousedown")+this._eventNamespace(),function(d){
                c._mouseDown=true;
                c._startX=c._getCoordinate(d,"pageX");
                c._startOffset=c._lastOffset=parseInt(c._innerWrapper.css("margin-left"),10)
            });
            this.addHandler(this.host,"dragstart",function(){
                return false
            });
            this.addHandler(this.host,this._getEvent("mousemove")+this._eventNamespace(),function(d){
                if(c._scrollEnabled(d)){
                    var f=c._startOffset+c._getCoordinate(d,"pageX")-c._startX;
                    if(!c.bounceEnabled){
                        f=c._getBounceOffset(f)
                    }
                    c._innerWrapper.css("margin-left",f);
                    c._setMoveDirection(f);
                    c._lastOffset=f;
                    d.preventDefault();
                    return false
                }
            ...
            this.addHandler(b(document),this._getEvent("mouseup")+this._eventNamespace(),function(d){
                if(c._movePermited){
                    c._dropTarget()
                }
        ...
        _performButtonsLayout:function(){
            var c=(this.host.width()-this._buttonsContainer.width())/2;
            this._buttonsContainer.css({
                position:"absolute",
                left:c+parseInt(this.buttonsOffset[0],10),
                top:this.host.height()-2*this._buttonsContainer.outerHeight()+parseInt(this.buttonsOffset[1],10)
            })
        },
