...
<!--                                            <th>Type</th>-->
		...
		<tr class="odd gradeX" id="slider_<?php echo $gall->id; ?>">
			...
			<td><img src="<?php echo base_url();?>/upload/category/<?php echo $gall->image;?>"   width="60px"  /></td>
				<td>
<?php 
         if($gall->status == 0){ $anc = 'Active'; }else{ $anc = 'Deactive'; }
                    ...
                    <a href="<?php echo site_url('admin/category/actdeact/'.$gall->id); ?>"  class="act-deact" title="<?php echo $gall->status; ?>" ><?php echo $anc; ?></a>
            ...
            <td class="text-right">
															...
															<a  href="<?php echo site_url("admin/category/delete_category/".$gall->id); ?>" class="btn btn-sm bg-danger-light dlt">
		...
		<?php endforeach;?>
			...
			<div class="col-xl-6 d-flex">
							<div class="card flex-fill">
								<div class="card-header bg-grey">
									<h4 class="card-title" id="card-title">Add Category</h4>
								</div>
								<div class="card-body">
									<form id="" method="post" action="<?php echo site_url('admin/category/create_category');?>" enctype="multipart/form-data">
										<div class="form-group row">
											<label class="col-lg-3 col-form-label">Title</label>
											<div class="col-lg-9">
												<input type="text" name="slider[name]" id="title" class="form-control" placeholder="Enter title" autocomplete="off" required>
											</div>
										</div>
									...
									<div class="form-group row">
											...
											<div class="col-lg-9">
												...
												<small>(Upload Images 100x100)</small>
												<br>
												<img src="" id="b_img" width="80px">
											</div>
										</div>
...
<script>
        ...
        {
                $.post("<?php echo site_url('admin/category/get_category');?>",{id:id},function(resp){
                 
               var parsedJson = $.parseJSON(resp);
               $('#title').val(parsedJson.name);
              
               var img = "<?php echo base_url();?>/upload/category/"+parsedJson.image;
                   $('#b_img').prop('src', img);
                   $('#bid').val(parsedJson.id);
                   $('#card-title').html('Edit Category');
                   $('#submit').html('Update');
