...
function session_out(){
	//validate form input
		...
		else
		{
			...
			$this->_render_page('admin/auth/session_out_login', $this->data);
		}
}
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			// render
			$this->load->view('common_user/header',$this->header);
			$this->_render_page('admin/auth/change_password', $this->data);
			$this->load->view('common_user/footer');
		}
		else
		{
			...
			// check to see if we are updating the user
			   if($this->ion_auth->update($user->id, $data))
			    { 
				    ...
				    if ($this->ion_auth->is_admin())
					{
						redirect('admin/auth/edit_user/1', 'refresh');
					}
					else
					{
						redirect('admin/auth/edit_user/1', 'refresh');
